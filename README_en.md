...
Current CI covers:
- Code quality checks: `ruff`, `black --check`, `mypy`
- Unit tests with `pytest` and coverage XML output
- Classifier backend smoke checks for `template`, `cnn`, and `embed`
- Integration smoke run for `scripts/run_pipeline.py --debug`
- Review UI API smoke checks for `/`, `/api/result`, `/api/grid`, and `/api/rendered`
